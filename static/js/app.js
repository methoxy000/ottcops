...
// Focused quick questions the user can click or drag into the prompt builder.
const PREBUILT_QUESTIONS = [
  {
    id: "q_overall_health",
    label: "Overall plant check",
    text: "What is the overall health status of the plant based on the image?",
  },
  {
    id: "q_trichome_window",
    label: "Trichome harvest window",
    text: "Estimate the harvest window from the trichome mix (clear/milky/amber).",
  },
  {
    id: "q_vpd_alignment",
    label: "VPD alignment",
    text: "Is the VPD on target for the current growth stage given the provided humidity/temperature?",
  },
  {
    id: "q_pest_scan",
    label: "Pest scan",
    text: "Do you see any pests or characteristic pest damage on the leaves or buds?",
  },
  {
    id: "q_feeding_ec_ph",
    label: "Feeding (EC/PH)",
    text: "Are EC and PH values appropriate based on visual leaf cues? Suggest adjustments if needed.",
  },
  {
    id: "q_canopy_airflow",
    label: "Canopy & airflow",
    text: "How is canopy structure, airflow, and light spread? Recommend any defoliation or training tweaks.",
  },
];
...
function cacheDom() {
  ...
  dom.prebuiltQuestionList = document.getElementById("prebuiltQuestionList");
...
function renderPrebuiltQuestions() {
  ...
  dom.prebuiltQuestionList.innerHTML = "";
  PREBUILT_QUESTIONS.forEach((item) => {
    const pill = document.createElement("button");
    pill.type = "button";
    pill.className = "pill-button";
    pill.textContent = item.label;
    pill.draggable = true;
    pill.dataset.text = item.text;
    pill.addEventListener("click", () => insertPromptText(item.text));
    pill.addEventListener("dragstart", (event) => {
      event.dataTransfer.setData("text/plain", pill.dataset.text);
      event.dataTransfer.setData("opencore-kind", "faq");
    });
